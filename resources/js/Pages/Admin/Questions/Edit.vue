<template>
    <AdminLayout :page-title="t('admin.edit_question') || 'Edit Question'">
        <Head :title="t('admin.edit_question') || 'Edit Question'" />
        <div class="max-w-4xl mx-auto">
            ...
            <div class="mb-8">
                <div class="flex items-center gap-3 mb-2">
                    ...
                    <h1 class="text-2xl font-bold text-gray-900">{{ t('admin.edit_question') || 'Edit Question' }}</h1>
                </div>
...
const form = useForm({
    ...
    question_ar: props.question.question_ar || '',
    explanation: props.question.explanation || '',
    explanation_ar: props.question.explanation_ar || '',
    points: props.question.points || 1,
    order: props.question.order || 1,
    answers: props.question.answers?.length > 0 
...
const submit = () => {
    form.put(route('admin.courses.lessons.questions.update', [props.course.slug || props.course.id, props.lesson.id, props.question.id]));
};

const confirmDelete = () => {
    if (confirm(t('questions.confirm_delete') || 'Are you sure you want to delete this question?')) {
        router.delete(route('admin.courses.lessons.questions.destroy', [props.course.slug || props.course.id, props.lesson.id, props.question.id]));
    }
};
</script>
