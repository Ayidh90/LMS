        ...
        <div class="min-h-screen bg-gray-50" :dir="direction">
            <div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
                <div class="px-4 py-6 sm:px-0">
                    <h1 class="text-3xl font-bold text-gray-900 mb-6">{{ t('student.dashboard') || 'Student Dashboard' }}</h1>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                        <div class="bg-white overflow-hidden shadow rounded-lg">
                            <div class="p-5">
                                ...
                                <div class="text-sm font-medium text-gray-500 mt-1">{{ t('student.enrolled_courses') || 'Enrolled Courses' }}</div>
                            </div>
                        </div>
                        ...
                        <div class="bg-white overflow-hidden shadow rounded-lg">
                            <div class="p-5">
                                ...
                                <div class="text-sm font-medium text-gray-500 mt-1">{{ t('student.completed') || 'Completed' }}</div>
                            </div>
                        </div>
                        ...
                        <div class="bg-white overflow-hidden shadow rounded-lg">
                            <div class="p-5">
                                ...
                                <div class="text-sm font-medium text-gray-500 mt-1">{{ t('student.in_progress') || 'In Progress' }}</div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="space-y-4">
                                <div v-for="enrollment in enrollments" :key="enrollment.id" class="border-b pb-4 last:border-b-0">
                                    <div class="flex justify-between items-start">
                                        <div>
                                            <h4 class="text-md font-medium text-gray-900">{{ enrollment.batch?.course?.translated_title || enrollment.batch?.course?.title || enrollment.course?.translated_title || enrollment.course?.title }}</h4>
                                            <p class="text-sm text-gray-500 mt-1">{{ t('courses.fields.instructor') }}: {{ enrollment.batch?.instructor?.name || enrollment.course?.instructor?.name || '-' }}</p>
                                            <div class="mt-2">
                                                <div class="w-full bg-gray-200 rounded-full h-2">
                                                    <div class="bg-indigo-600 h-2 rounded-full" :style="`width: ${enrollment.progress || 0}%`"></div>
                                                </div>
                                                <span class="text-xs text-gray-500 mt-1">{{ enrollment.progress || 0 }}% {{ t('student.complete') || 'Complete' }}</span>
                                            </div>
                                        </div>
                                ...
                                <div v-if="enrollments.length === 0" class="text-center py-8 text-gray-500">
                                    {{ t('student.no_enrollments') || 'You haven\'t enrolled in any courses yet.' }}
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="px-4 py-5 sm:p-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                ...
                                <div v-if="availableCourses.length === 0" class="col-span-full text-center py-8 text-gray-500">
                                    {{ t('courses.no_courses') || 'No courses available at the moment.' }}
                                </div>
                            </div>
                        </div>
                    </div>
